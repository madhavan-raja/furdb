...
impl Table {
    pub async fn get_sortfile(&mut self, column: &Column) -> Result<Sortfile, Box<dyn Error>> {
    ...
    pub async fn generate_sortfile(&mut self, column: &Column) -> Result<(), Box<dyn Error>> {
        let current_sortfile = self.get_sortfile(column).await?;

        self.save_sortfile(&current_sortfile)
    }

    pub async fn generate_sortfiles(&mut self, columns: &[Column]) -> Result<(), Box<dyn Error>> {
    ...
    pub fn clear_sortfile(&self, column: &Column) -> Result<(), Box<dyn Error>> {
    ...
    pub fn clear_sortfiles(&self, columns: &[Column]) -> Result<(), Box<dyn Error>> {
        for column in columns {
